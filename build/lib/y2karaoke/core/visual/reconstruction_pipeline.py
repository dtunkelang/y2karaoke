    ...
    for i, line in enumerate(lines):
        ...
        if dur < 1.0 and wc < 4:
            for j, other in enumerate(lines):
                ...
                if line["lane"] == other["lane"] and abs(line["first"] - other["first"]) < 3.0:
...
def _sequence_by_visual_neighborhood(lines: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    adj = {i: set() for i in range(len(lines))}
