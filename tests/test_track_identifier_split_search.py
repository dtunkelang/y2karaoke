...
def test_try_split_search_selects_best_candidate(monkeypatch):
    ...
    monkeypatch.setattr(identifier, "_score_split_candidate", lambda c, a, t, q: 20 if a == "Artist2" else 10)
...
def test_try_split_search_returns_none_when_no_candidates(monkeypatch):
    ...
    monkeypatch.setattr(identifier, "_try_artist_title_splits", lambda query: [("Artist", "Title")])
    monkeypatch.setattr(identifier, "_query_musicbrainz", lambda query, artist, title: [])
...
def test_try_split_search_skips_when_no_best_candidate(monkeypatch):
    ...
    monkeypatch.setattr(identifier, "_find_best_with_artist_hint", lambda recordings, query, artist: None)
