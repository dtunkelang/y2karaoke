...
from y2karaoke.core.audio_effects import AudioProcessor, process_audio, shift_key, change_tempo
...
class TestAudioProcessor:
    ...
    def test_process_audio_validates_tempo(self):
        ...
        with patch('y2karaoke.core.audio_effects.validate_tempo') as mock_validate:
            ...
            with pytest.raises(ValidationError):
                processor.process_audio("/input.wav", "/output.wav", tempo_multiplier=5.0)
    ...
    @patch('y2karaoke.core.audio_effects.Path')
    def test_process_audio_pitch_shift(self, mock_path, mock_load, mock_write, mock_pitch_shift):
    ...
    @patch('y2karaoke.core.audio_effects.librosa.load')
    @patch('y2karaoke.core.audio_effects.Path')
    def test_process_audio_tempo_change(self, mock_path, mock_load, mock_write, mock_time_stretch):
        ...
        processor = AudioProcessor()
        result = processor.process_audio("/input.wav", "/output.wav", tempo_multiplier=1.5)
    ...
    def test_shift_key_delegates_to_process_audio(self):
        ...
        with patch.object(processor, 'process_audio') as mock_process:
            ...
            mock_process.assert_called_once_with("/input.wav", "/output.wav", semitones=3)
    ...
    def test_change_tempo_delegates_to_process_audio(self):
        ...
        with patch.object(processor, 'process_audio') as mock_process:
            ...
            assert result == "/output.wav"
            mock_process.assert_called_once_with("/input.wav", "/output.wav", tempo_multiplier=0.8)
...
class TestAudioEffectsConvenienceFunctions:
    ...
    def test_process_audio_function(self, mock_processor_class):
        ...
        result = process_audio("/input.wav", "/output.wav", semitones=2, tempo_multiplier=1.2)
    ...
    def test_change_tempo_function(self, mock_processor_class):
        ...
        assert result == "/output.wav"
        mock_processor_class.assert_called_once()
        mock_processor.change_tempo.assert_called_once_with("/input.wav", "/output.wav", 0.75)
...
class TestAudioEffectsErrorHandling:
    ...
    def test_invalid_tempo_handling(self):
        ...
        with patch('y2karaoke.core.audio_effects.validate_tempo') as mock_validate:
            ...
            with pytest.raises(ValidationError):
                processor.process_audio("/input.wav", "/output.wav", tempo_multiplier=10.0)
...
class TestAudioEffectsValidation:
    ...
    def test_convenience_functions_use_processor(self):
        ...
        with patch('y2karaoke.core.audio_effects.AudioProcessor') as mock_processor_class:
            mock_processor = Mock()
            mock_processor_class.return_value = mock_processor
