...
def _install_raw_frame_confidence_doubles(monkeypatch) -> dict[str, _FakeCap | None]:
    captured: dict[str, _FakeCap | None] = {"cap": None}
    ...
    monkeypatch.setattr(_MODULE, "get_ocr_engine", lambda: _FakeOCR())
...
def test_collect_raw_frames_with_confidence_uses_sampled_retrieve(monkeypatch) -> None:
    captured = _install_raw_frame_confidence_doubles(monkeypatch)
