...
def test_align_pipeline_writes_trace_on_low_dtw_forced_fallback(monkeypatch, tmp_path):
    lines = [
        Line(words=[Word(text=f"w{i}", start_time=float(i), end_time=float(i) + 0.5)])
        for i in range(20)
    ]
    ...
    trace_path = tmp_path / "whisper_stages.json"

    monkeypatch.setenv("Y2K_TRACE_WHISPER_STAGES_JSON", str(trace_path))
    monkeypatch.setenv("Y2K_TRACE_WHISPER_LINE_RANGE", "1-3")
    monkeypatch.setattr(
        "y2karaoke.core.components.whisper.whisper_integration_align.align_lines_with_whisperx",
        lambda *_args, **_kwargs: (
            forced_lines,
