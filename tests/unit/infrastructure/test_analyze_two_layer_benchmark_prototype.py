...
_MODULE_PATH = (
    ...
    / "tools"
    / "analyze_two_layer_benchmark_prototype.py"
)
_SPEC = importlib.util.spec_from_file_location(
    "analyze_two_layer_benchmark_prototype_module", _MODULE_PATH
)
...
def test_analyze_two_layer_benchmark_prototype_adjusts_aggregate_counts(
    tmp_path: Path,
) -> None:
    timing_report = {
        "artist": "Artist",
    ...
    benchmark_report = {
        "aggregate": {
            "line_count_total": 20,
            "agreement_count_total": 4,
            "agreement_eligible_lines_total": 10,
            "agreement_matched_anchor_lines_total": 4,
            "agreement_good_lines_total": 3,
            "agreement_bad_lines_total": 1,
            "agreement_warn_lines_total": 1,
            "agreement_severe_lines_total": 0,
            "agreement_coverage_ratio_total": 0.4,
    ...
    assert result["baseline"]["agreement_eligible_lines_total"] == 10
    assert result["prototype"]["agreement_count_total"] == 6
    assert result["prototype"]["agreement_eligible_lines_total"] == 11
    assert result["prototype"]["agreement_matched_anchor_lines_total"] == 6
    assert result["prototype"]["agreement_good_lines_total"] == 5
    assert result["prototype"]["agreement_bad_lines_total"] == 1
    assert result["prototype"]["agreement_coverage_ratio_total"] == 0.5455
