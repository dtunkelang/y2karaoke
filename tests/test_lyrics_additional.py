...
def test_detect_and_apply_offset_large_delta_applies(monkeypatch):
    monkeypatch.setattr("y2karaoke.core.alignment.detect_song_start", lambda *_: 20.0)
...
def test_detect_offset_with_issues_respects_manual(monkeypatch):
    monkeypatch.setattr("y2karaoke.core.alignment.detect_song_start", lambda *_: 10.0)
...
def test_get_lyrics_simple_whisper_failure(monkeypatch):
    ...
    monkeypatch.setattr(
        lyrics, "_fetch_lrc_text_and_timings", lambda *a, **k: (lrc_text, line_timings, "src")
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        lyrics, "create_lines_from_lrc", lambda *a, **k: [_line("First"), _line("Second")]
    )
    monkeypatch.setattr(lyrics, "_refine_timing_with_audio", lambda *a, **k: [_line("First")])

    def raise_whisper(*args, **kwargs):
        raise RuntimeError("whisper down")

    monkeypatch.setattr(lyrics, "_apply_whisper_alignment", raise_whisper)
...
def test_get_lyrics_with_quality_lrc_path(monkeypatch):
    ...
    monkeypatch.setattr(lyrics, "_refine_timing_with_quality", lambda *a, **k: ([_line("First")], "onset_refined"))
    monkeypatch.setattr(
        lyrics,
        "_apply_whisper_alignment",
        lambda *a, **k: ([_line("First")], ["fix"]),
    ...
    lines, meta, report = lyrics.get_lyrics_with_quality(
        "Title",
        "Artist",
        vocals_path="vocals.wav",
        use_whisper=True,
...
def test_get_lyrics_with_quality_fallback_to_genius(monkeypatch):
    ...
    monkeypatch.setattr(
        lyrics,
        "_fetch_genius_with_quality_tracking",
        lambda *a, **k: ([("Hello", "Singer")], SongMetadata(singers=["Singer"], is_duet=False)),
    )
    monkeypatch.setattr(
        lyrics, "create_lines_from_lrc", lambda *a, **k: [_line("Hello")]
    )
...
def test_lyrics_processor_delegates_to_get_lyrics_simple(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(lyrics, "get_lyrics_simple", fake_get_lyrics_simple)

    lines, metadata = processor.get_lyrics(title="Title", artist="Artist", romanize=False)
...
def test_get_lyrics_convenience_calls_simple(monkeypatch):
    monkeypatch.setattr(
        lyrics, "get_lyrics_simple", lambda *a, **k: ([_line("Hi")], SongMetadata(singers=[], is_duet=False))
    )
