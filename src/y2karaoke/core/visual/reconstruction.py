...
def _extrapolate_mirrored_lane_cycles(
    ...
    for i in range(n):
        a = entries[i]
        if not _is_candidate_for_mirrored_cycle(a):
            continue

        for j in range(i + 1, n):
            b = entries[j]
            candidate = _mirrored_cycle_candidate(a, b)
...
def _is_candidate_for_mirrored_cycle(entry: dict[str, Any]) -> bool:
    if _is_short_refrain_entry(entry):
        return False
...
def _mirrored_cycle_candidate(
    ...
    if not _is_candidate_for_mirrored_cycle(b):
        return None
    if _is_same_lane(a, b):
        return None
    if text_similarity(str(a.get("text", "")), str(b.get("text", ""))) < 0.97:
        return None
