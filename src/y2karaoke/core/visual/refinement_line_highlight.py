...
def _build_frame_activity_series(
    vals: List[Dict[str, Any]], line_frames: List[FrameTriplet]
    ...
    return frame_times, frame_present, frame_activity


def _fallback_highlight_result(
...
def detect_line_highlight_with_confidence(
    ...
    *,
    ...
    if not vals:
        return None, None, 0.0
    frame_times, frame_present, frame_activity = _build_frame_activity_series(
        vals, line_frames
    )
    ...
    return _fallback_highlight_result(
        vals,
        min_start_time=min_start_time,
        detect_sustained_onset=detect_sustained_onset,
        detect_highlight_with_confidence=detect_highlight_with_confidence,
    )
