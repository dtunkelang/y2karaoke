...
class AudioSeparator:
    ...
    def separate_vocals(self, audio_path: str, output_dir: Optional[str] = None) -> Dict[str, str]:
        ...
        if any(marker in audio_filename for marker in ['vocals', 'instrumental', 'drums', 'bass', 'other']):
            raise SeparationError(f"Cannot separate an already-separated file: {audio_path}")
        
        # Check for existing cached stems first
        effective_base = Path(audio_path).stem
        existing_vocals = self._find_existing_file(output_dir, f"{effective_base}*_(Vocals)_*.wav")
        existing_instrumental = self._find_existing_file(output_dir, f"{effective_base}*_instrumental.wav")
...
def separate_vocals(audio_path: str, output_dir: str = ".") -> dict:
    ...
    if instrumental_path is None and vocals_path:
        ...
        if non_vocal_files:
            ...
            logger.debug(f"Mixing {len(non_vocal_files)} stems into instrumental track...")
