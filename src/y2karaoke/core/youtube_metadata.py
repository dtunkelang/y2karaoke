...
def _parse_artist_title_from_video_title(video_title: str, uploader: str = "") -> Tuple[str, str]:
    ...
    cleaned = video_title
    patterns_to_remove = [
        r'\s*\(Official\s*(Music\s*)?Video\)', r'\s*\(Official\s*Audio\)',
        r'\s*\(Official\s*Lyric\s*Video\)', r'\s*\(Lyric\s*Video\)',
        r'\s*\(Lyrics?\)', r'\s*\(Audio\)', r'\s*\(Visualizer\)',
        r'\s*\(Remaster(ed)?\s*\d*\)', r'\s*\(Live\)',
        r'\s*\[Official\s*(Music\s*)?Video\]', r'\s*\[Official\s*Audio\]',
        r'\s*\[4K\]', r'\s*\[HD\]', r'\s*\[HQ\]', r'\s*\[\d+K\]',
        r'\s*\(4K\)', r'\s*\(HD\)', r'\s*\(HQ\)', r'\s*\(\d+K\)',
        r'\s*【[^】]*】', r'\s*M/?V\s*$', r'\s*\(M/?V\)',
    ]
...
def _clean_uploader_name(uploader: str) -> str:
    ...
    if not uploader:
        return ""
    artist = uploader
    suffixes = ['Official', 'VEVO', 'Records', 'Music', 'Channel', '- Topic', ' - Topic', 'TV', 'Band', 'Entertainment']
...
def extract_metadata_from_youtube(url: str) -> Dict[str, str]:
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if yt_artist and yt_track:
            return {'artist': yt_artist, 'title': yt_track, 'video_id': info.get('id', extract_video_id(url))}
        
        artist, title = _parse_artist_title_from_video_title(video_title, uploader)
        if artist and title:
            return {'artist': artist, 'title': title, 'video_id': info.get('id', extract_video_id(url))}
        ...
        return {'artist': artist or 'Unknown', 'title': title or video_title, 'video_id': info.get('id', extract_video_id(url))}
