...
- current `Without Me` diagnostic from `benchmarks/results/20260321T004829Z/06_eminem-without-me-repeated-hook_timing_report.json`:
  ...
  - exact first read: start with the first-pass late-pull gates in `whisper_mapping_post_segment_pull.py` around the `min_sim_required` / `best_token_overlap` / `late_by` / `window_start` block, because that is the narrowest code path still consistent with the current line-2 evidence
